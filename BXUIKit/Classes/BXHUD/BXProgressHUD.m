...
#import "BXProgressHUD.h"
	...
	#define BXAUTORELEASE(exp) exp
	#define BXRELEASE(exp) exp
	#define BXRETAIN(exp) exp
#else//如果是MRC状态下
	#define BXAUTORELEASE(exp) [exp autorelease]
	#define BXRELEASE(exp) [exp release]
	#define BXRETAIN(exp) [exp retain]
#endif

#define labelMargin 20.0f //label与HUD的边缘间距

static const CGFloat kPadding = 10.f;
static UIFont *kBXHUDLabelFont = nil;
static UIFont *kBXHUDDetailsLabelFont = nil;
static UIImage *kBXHUDRefreshCircleImage= nil;
static UIImage *kBXHUDRefreshLogoImage= nil;
...
@property (BXSTRONG) UIView *indicator;
@property (BXSTRONG) NSTimer *graceTimer;
@property (BXSTRONG) NSTimer *minShowTimer;
@property (BXSTRONG) NSDate *showStarted;
@property (BXSTRONG) UIImageView *imgView;//gome 红圈
...
+ (BXProgressHUD *)HUDAddedTo:(UIView *)view {
    ...
    UIImageView *imgView2 = [[UIImageView alloc] initWithImage:kBXHUDRefreshLogoImage];
    ...
    hud.customView = v;
    hud.mode = BXProgressHUDModeCustomView;
    [view addSubview:hud];
    return BXAUTORELEASE(hud);
}

+ (BXProgressHUD *)showHUDAddedTo:(UIView *)view animated:(BOOL)animated {
	BXProgressHUD *hud = [BXProgressHUD HUDAddedTo:view];
	[hud show:animated];
	return BXAUTORELEASE(hud);
}
...
+ (BOOL)hideHUDForView:(UIView *)view animated:(BOOL)animated {
	BXProgressHUD *hud = [BXProgressHUD HUDForView:view];
	if (hud != nil) {
...
+ (BXProgressHUD *)HUDForView:(UIView *)view {
	BXProgressHUD *hud = nil;
	NSArray *subviews = view.subviews;
	Class hudClass = [BXProgressHUD class];
	for (UIView *aView in subviews) {
		if ([aView isKindOfClass:hudClass]) {
			hud = (BXProgressHUD *)aView;
		}
	}
	return hud;
}
...
- (id)initWithFrame:(CGRect)frame {
	...
	if (self) {
		...
		self.animationType = BXProgressHUDAnimationFade;
		self.mode = BXProgressHUDModeIndeterminate;
		self.labelText = nil;
		self.detailsLabelText = nil;
		self.opacity = 0.8f;
        self.color = nil;
        self.labelFont = kBXHUDLabelFont==nil?[UIFont systemFontOfSize:16]:kBXHUDLabelFont;
		self.detailsLabelFont = kBXHUDDetailsLabelFont==nil?[UIFont systemFontOfSize:12]:kBXHUDDetailsLabelFont;
		self.xOffset = 0.0f;
		self.yOffset = -30.0f;
...
- (void)showUsingAnimation:(BOOL)animated {
	self.alpha = 0.0f;
	if (animated && animationType == BXProgressHUDAnimationZoom) {
	...
	if (animated) {
		...
		self.alpha = 1.0f;
		if (animationType == BXProgressHUDAnimationZoom) {
...
- (void)showWhileExecuting:(SEL)method onTarget:(id)target withObject:(id)object animated:(BOOL)animated {
	...
	targetForExecution = BXRETAIN(target);
	objectForExecution = BXRETAIN(object);
...
- (void)showAnimated:(BOOL)animated whileExecutingBlock:(dispatch_block_t)block onQueue:(dispatch_queue_t)queue
	 completionBlock:(BXProgressHUDCompletionBlock)completion {
...
- (void)updateIndicators {
	...
	BOOL isRoundIndicator = [indicator isKindOfClass:[BXRoundProgressView class]];
	
	if (mode == BXProgressHUDModeIndeterminate &&  !isActivityIndicator) {
		// Update to indeterminate indicator
		[indicator removeFromSuperview];
		self.indicator = BXAUTORELEASE([[UIActivityIndicatorView alloc]
										 initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhiteLarge]);
		[(UIActivityIndicatorView *)indicator startAnimating];
		[self addSubview:indicator];
	}
	else if (mode == BXProgressHUDModeDeterminate || mode == BXProgressHUDModeAnnularDeterminate) {
		if (!isRoundIndicator) {
			// Update to determinante indicator
			[indicator removeFromSuperview];
			self.indicator = BXAUTORELEASE([[BXRoundProgressView alloc] init]);
			[self addSubview:indicator];
		}
		if (mode == BXProgressHUDModeAnnularDeterminate) {
			[(BXRoundProgressView *)indicator setAnnular:YES];
		}
	} 
	else if (mode == BXProgressHUDModeCustomView && customView != indicator) {
		// Update custom view indicator
		[indicator removeFromSuperview];
		self.indicator = customView;
		[self addSubview:indicator];
	} else if (mode == BXProgressHUDModeText) {
		[indicator removeFromSuperview];
		self.indicator = nil;
	}
}
...
@implementation BXRoundProgressView {
	CGFloat _progress;
	BOOL _annular;
}
